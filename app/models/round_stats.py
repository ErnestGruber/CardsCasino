...
class RoundStats(Base):
    ...
    total_bones = Column(Float, nullable=False, default=0)  # Общий BONES в раунде
    total_not = Column(Float, nullable=False, default=0)  # Общий NOT в раунде
    total_bank = Column(Float, nullable=False, default=0)  # Общий банк
    admin_fee = Column(Float, nullable=False)  # Комиссия администратора
    bones_coefficient = Column(Float, nullable=False, default=0)  # Коэффициент для BONES
    not_coefficient = Column(Float, nullable=False, default=0)  # Коэффициент для NOT
    winner_card_id = Column(Integer, ForeignKey('card.id'), nullable=True)  # ID победившей карточки
